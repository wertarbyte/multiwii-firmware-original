...
void i2CLedRingState() {
  ...
  if (state == 0) {
    ...
    b[1]= 1;
    if (flag.ARMED) b[2]= 1; else b[2]= 0;
    i2c_rep_start(LED_RING_ADDRESS<<1);
    for(uint8_t i=0;i<3;i++)
      i2c_write(b[i]);
    i2c_stop();
...
void led_flasher_autoselect_sequence() {
  ...
  if (0) {
    ...
    led_flasher_set_sequence(flag.ARMED ? LED_FLASHER_SEQUENCE_ARMED : LED_FLASHER_SEQUENCE);
    #else
...
void auto_switch_landing_lights(void) {
  if (rcOptions[BOXLLIGHTS]
	...
	|| (sonarAlt >= 0 && sonarAlt <= LANDING_LIGHTS_AUTO_ALTITUDE && flag.ARMED)
  #endif
  ) {
